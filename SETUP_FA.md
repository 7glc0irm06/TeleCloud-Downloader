...
اگر روی سرور لینوکسی هستید و می‌خواهید با یک دستور سرویس را بالا بیاورید، می‌توانید از `start.sh` استفاده کنید:
...
- به مسیر `/root/bot` می‌رود
- اگر `cookies_enabled.json` وجود نداشته باشد، آن را می‌سازد
- اگر `rclone.conf` وجود نداشته باشد، آن را از `/root/.config/rclone/rclone.conf` کپی می‌کند
- دستور `docker compose up -d` را اجرا می‌کند

نکته مهم:

- این اسکریپت فرض می‌کند مسیر پروژه شما `/root/bot` است.
- همچنین فرض می‌کند فایل `/root/.config/rclone/rclone.conf` روی سرور موجود است.
- اگر مسیرهای شما متفاوت است، از روش دستی Docker (بخش‌های بالا) استفاده کنید.
